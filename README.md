# Diet-Tracker-Rpi
Diet-Tracker consists of a basic website front-end, website back-end, and Python code that runs on a Raspberry Pi. This repository holds code for the Raspberry Pi.
...
The back end was written in python using the Django framework. The front end was written in plain Javascript, HTML, and CSS. The user is presented with a controllable graph, which provides options to show how much energy, fats, protein, and carbs were consumed on a regular basis. The user may select from a range of timeframes (daily, weekly, monthly).
